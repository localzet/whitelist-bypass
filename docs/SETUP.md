...
Скачайте нужные файлы из [GitHub Releases](https://github.com/localzet/vconnect/releases).
...
2. Скачайте бинарники из [GitHub Releases](https://github.com/localzet/vconnect/releases) и сложите их в одну папку - например `/usr/local/bin/`:
...
./headless-vk-bot \
  ...
  --vk-cookies /etc/vconnect/cookies-vk.json \
  --tm-cookies /etc/vconnect/cookies-yandex.json \
  --wb-cookies /etc/vconnect/cookies-wbstream.json \
  --dion-cookies /etc/vconnect/cookies-dion.json
```
...
```ini
[Unit]
Description=Headless VK bot (vconnect)
...
Альтернатива systemd. Готовый образ публикуется в GHCR (`ghcr.io/localzet/vconnect-bot`) - под капотом тот же `headless-vk-bot` плюс четыре creator-бинарника. Поддерживаемые архитектуры: `linux/amd64`, `linux/arm64`, `linux/386`.

```sh
mkdir wlb-bot && cd wlb-bot
curl -O https://raw.githubusercontent.com/localzet/vconnect/main/headless/docker/docker-compose.yml
curl -L https://raw.githubusercontent.com/localzet/vconnectadless/docker/.env.example -o .env
...
1. Скачайте и установите `vconnect.apk`
...
Файл `vconnect-proxy.ipa` из [GitHub Releases](https://github.com/localzet/vconnect/releases) является **unsigned IPA**. Его нельзя просто открыть как APK на Android: iOS требует подписать приложение Apple ID или developer-сертификатом перед установкой.

Варианты установки:

- **AltStore / SideStore** - удобный вариант для обычного Apple ID. Установите AltStore на iPhone, импортируйте `vconnect-proxy.ipa` и дождитесь подписи. Ограничение бесплатного Apple ID: приложение нужно обновлять примерно раз в 7 дней.
...
1. Установите `vconnect-proxy.ipa` одним из способов выше. Подробная сторонняя инструкция по unsigned IPA: [habr.com/ru/articles/1013990/](https://habr.com/ru/articles/1013990/).
2. Установите любое VPN-приложение с поддержкой SOCKS5 (Happ, Shadowrocket, Streisand и т.п.).
3. Откройте VConnect, выберите режим туннеля (**DC** или **Video**), вставьте ссылку на звонок и нажмите **Go**. Headless-creator подстроится под выбранный режим автоматически.
4. Дождитесь статуса "Tunnel Active". Приложение покажет адрес SOCKS5 прокси (например `socks5://user:pass@127.0.0.1:1081`).
5. Скопируйте параметры SOCKS5 из VConnect.
...
1. В VConnect дождитесь "Tunnel Active" и нажмите кнопку **Copy v2ray URL** - в буфер обмена скопируется ссылка вида `socks://...@127.0.0.1:1081#WLB-1081`.

   ![VConnect: Copy v2ray URL](res/ios_interface.jpeg)
...
> Приложение работает без NetworkExtension - это позволяет устанавливать его через бесплатный Apple Developer аккаунт (без подписки $99/год). Обратная сторона: порт SOCKS5 может меняться между запусками VConnect - если предыдущий порт занят, приложение выбирает свободный. Если в Happ туннель перестал работать после перезапуска - удалите сервер `WLB-...` и заново нажмите **Copy v2ray URL** + **Import from clipboard**.
...
- Или нажмите в VConnect кнопку **"Open in Telegram"** для автоматической настройки
...
Из [GitHub Releases](https://github.com/localzet/vconnect/releases):

- `VConnect Joiner-<version>-x64.exe` / `-ia32.exe` - Windows (portable, без установщика)
- `VConnect Joiner-<version>-x86_64.AppImage` - Linux
- `VConnect Joiner-<version>-arm64.dmg` / `-x64.dmg` - macOS
...
| Linux | root для TUN-режима | Запустите `xhost +SI:localuser:root` и затем `sudo -E ./VConnect\ Joiner-*.AppImage --no-sandbox`. Без root доступен только режим **SOCKS5 only** |
| macOS | root для TUN-режима | `sudo "/Applications/VConnect Joiner.app/Contents/MacOS/VConnect Joiner"`. Без sudo доступен только режим **SOCKS5 only** |
...
Скачайте бинарник из [GitHub Releases](https://github.com/localzet/vconnects):
