...
```text
--egress-config /etc/vconnect/egresses.json
...
- Telemost headless creator поддерживает server-side service-control режим: поднимает Telemost service-call, принимает cookies/session requests через звонок, хранит cookies в vault и запускает work-call через существующие headless creators. Docker creator-service запускает именно этот режим.
- Отдельный `Dockerfile.creator-service`, Compose manifest и GHCR workflow публикуют server-only image `vconnect-creator-service` для amd64/arm64. Образ по умолчанию запускает creator-service, не содержит bot entrypoint, поддерживает Docker secret для vault key и сообщает health только после появления свежей service-call ссылки. Один container/service-call принимает CSV allowlist `USER_IDS`; `ServiceHandler` проверяет ID каждого control message и разделяет vault/work-session state по пользователю. `MAX_ACTIVE_USERS` резервируется до запуска work factory, поэтому превышение лимита не создаёт даже кратковременный лишний creator-процесс.
...
Android/headless service-control flow реализован. Следующая клиентская итерация должна заменить ручной путь к cookie-файлу в desktop UI на безопасный локальный экспорт/выбор cookies.
...
Android lifecycle orchestration теперь реализован через `ServiceJoinController`: service destination подключается к Telemost service-call, получает `SessionReady`, полностью останавливает служебный native relay и только затем запускает обычный headless joiner для work-call. Тот же controller используется из Activity и foreground service. Destination хранит флаг service-control и целевую work platform; ручной `egressId` остаётся частью destination. При переподключении service-call relay повторяет тот же idempotent session request, поэтому потеря ответа не создаёт дополнительный work-call.

Android ищет cookie-файл только в app-private `files/service-cookies/<userId>/<platform>.json`. Если локальная сессия отсутствует, service flow использует ранее сохранённые server-side cookies либо завершается серверной ошибкой. Cookie payload и содержимое файла не записываются в preferences или логи.

Android Yandex login/export UI реализован в `YandexAccountFragment`. WebView открывает официальный Passport login, а явное действие пользователя собирает cookies только для Yandex/Telemost origins и требует auth cookie `Session_id`. JSON сохраняется атомарно с owner-only permissions в user-scoped app-private каталоге. Значения cookies не попадают в UI, preferences или logs; Android Auto Backup отключён, чтобы vault и WebView profile не уходили в cloud backup. Удаление локальной service session полностью очищает общий WebView cookie jar: это требует повторного входа в другие WebView-сессии, но исключает экспорт cookies предыдущего пользователя.

Desktop joiner больше не принимает user ID и cookie path из renderer. Main process создаёт стабильный UUID, использует отдельную persistent Electron partition для Yandex login, проверяет `Session_id`, атомарно сохраняет owner-only cookie JSON под `userData/service/<userId>/` и сам добавляет путь в аргументы child process. Renderer получает только статус наличия сессии; cookie path редактировать нельзя и в process log он редактируется как `<private-cookie-file>`.

Цветовая схема клиентов переключается между `Mint / Cyan` и `Violet`, default — cyan. Desktop joiner и creator хранят выбор локально и используют CSS tokens. Android использует `AccentMode`, Material theme overlay и theme attributes во всех active-state drawables; старый бордовый `accent_emerald` заменён cyan-палитрой.
