...
Следующая итерация должна добавить реальный `creator-service`, который поднимет service-call, подключит `SetOnSessionCreate` к `Orchestrator` и `ProcessWorkCallFactory`, а затем будет создавать рабочий звонок только после получения `MsgSessionCreate` по служебному звонку, а не через обязательный внешний API.
