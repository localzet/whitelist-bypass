...
Следующая итерация должна добавить поддержку отправки `MsgCookieSubmit` и `MsgSessionCreate` на joiner-стороне, чтобы клиент мог войти в Яндекс локально, передать cookies через service-call и затем автоматически переподключиться к полученному work-call.
