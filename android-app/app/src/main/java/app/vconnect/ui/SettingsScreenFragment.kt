...
import app.vconnect.App
import app.vconnect.R
import app.vconnect.tunnel.SplitTunnelingMode
import app.vconnect.tunnel.CallPlatform
import app.vconnect.tunnel.ServiceCookieStore
import app.vconnect.tunnel.TunnelMode
import app.vconnect.util.Callback
import app.vconnect.util.AccentMode
import app.vconnect.util.ParamCallback
import app.vconnect.util.Prefs
import app.vconnect.util.ThemeMode
...
class SettingsScreenFragment : Fragment(R.layout.fragment_settings_screen) {
    ...
    private fun buildTunnelSection(): View {
        ...
        val tunnelMode = if (Prefs.tunnelMode in TunnelMode.globalOptions) Prefs.tunnelMode else TunnelMode.VIDEO
        if (tunnelMode != Prefs.tunnelMode) Prefs.tunnelMode = tunnelMode
        addRow(card, R.drawable.ic_setting_tunnel, getString(R.string.settings_row_tunnel_mode), null, tunnelMode.label) {
            ChoiceActionSheet.show(
                manager = parentFragmentManager,
                title = getString(R.string.settings_row_tunnel_mode),
                options = TunnelMode.globalOptions.map { ChoiceActionSheet.Option(it.name, it.label) },
                selectedId = tunnelMode.name,
            ) { picked ->
