...
class MainFragment : Fragment(R.layout.fragment_main_screen) {
    ...
    private fun editTunnelMode(config: CallConfig) {
        val current = config.effectiveTunnelMode(Prefs.tunnelMode)
        ChoiceActionSheet.show(
            manager = parentFragmentManager,
            title = getString(R.string.settings_row_tunnel_mode),
            options = TunnelMode.optionsFor(config.platform).map { ChoiceActionSheet.Option(it.name, it.label) },
    ...
    private fun refreshStats() {
        ...
        val active = Prefs.activeDestination
        val mode = active?.effectiveTunnelMode(Prefs.tunnelMode) ?: Prefs.tunnelMode
