...
data class CallConfig(
    ...
    fun effectiveTunnelMode(defaultMode: TunnelMode): TunnelMode = (tunnelMode ?: defaultMode).forPlatform(platform)

    val routeLabel: String
        get() = if (serviceControl) {
            val work = workPlatform ?: CallPlatform.TELEMOST
            val egress = egressId?.takeIf { it.isNotBlank() } ?: "choose"
            "Service -> ${platformLabelFor(work)} / $egress"
        } else {
            egressId?.takeIf { it.isNotBlank() }?.let { "$url / $it" } ?: url
        }
    ...
    val routeGlyph: String get() = if (serviceControl) "SVC" else platformGlyph
    ...
    companion object {
        ...
        fun suggestNameFor(url: String): String {
            ...
            val label = platformLabelFor(platform).let { if (platform == CallPlatform.VK) "$it call" else it }
            return label
        }

        private fun platformLabelFor(platform: CallPlatform): String = when (platform) {
            CallPlatform.VK -> "VK"
            CallPlatform.TELEMOST -> "Telemost"
            CallPlatform.WBSTREAM -> "WB Stream"
            CallPlatform.DION -> "DION"
