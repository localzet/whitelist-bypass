...
enum class TunnelMode(val label: String, val relayArg: String, val isPion: Boolean) {
    ...
    companion object {
        val globalOptions: List<TunnelMode> = listOf(VIDEO)

        fun optionsFor(platform: CallPlatform): List<TunnelMode> =
            entries.filter { it == VIDEO || (platform != CallPlatform.TELEMOST && platform != CallPlatform.DION) }
    }
}
